...
(function(C){
    ...
    var CMAC = C.algo.CMAC = Base.extend({
        ...
        /**
         * Authenticate messageUpdate.
         * 
         * @param {messageUpdate} message to be authenticated.
         * describe: If messageUpdate sigBytes is more than 16, encrypto the block(16 bytes) every time 
         *         until messageUpdate sigBytes is less than 16 
         */
        update: function (messageUpdate) {
            ...
            var bsize = this._const_Bsize; // block size
            ...
            // every time get 16 bytes, and encrypto. 
            while(buffer.sigBytes > bsize){ 
                var M_i = ext.shiftBytes(buffer, bsize); // get one block(16 bytes one time), buffer's length sub the top 16 bytes every time
                ext.xor(this._x, M_i);
                this._x.clamp();
                this._x = aesBlock(this._K, this._x); // encryto the block
         ...
         * MAC Generation
         * 
         * @param {messageUpdate} Data to be encrypted.
         * describe: K1, K2 for 128-bit subkeys.If buffer.sigBytes is 16, 
         *     directly M_last xor this._K1, or  padding(M_last), operate then M_last xor this._K1     
        ...
        finalize: function (messageUpdate) {
            ...
            var M_last = buffer.clone(); // the last block xor-ed with K1 or K2 
...
(function(C){
    ...
    var EAX = C.EAX = Base.extend({
        ...
        initCrypt: function(isEncrypt, nonce){
            var self = this;
            self._tag = self._mac.finalize();  // finailize the header, default update(one)
